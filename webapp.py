...
from shorts_service import generar_short_desde_url
...
def _run_job(job_id: str, video_url: str, api_key: str, output_filename: str) -> None:
    _set_job(job_id, status="queued", message="Jobbet ar koat.", updatedAt=time.time())
    try:
        result = generar_short_desde_url(
            video_url=video_url,
            api_key=api_key,
