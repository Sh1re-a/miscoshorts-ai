...
class FreeDiskBytesTests(unittest.TestCase):
    def test_returns_free_bytes(self) -> None:
        usage = MagicMock()
        usage.free = 123456
        with patch("shutil.disk_usage", return_value=usage):
            self.assertEqual(_free_disk_bytes(Path(".")), 123456)

    def test_returns_none_on_os_error(self) -> None:
        with patch("shutil.disk_usage", side_effect=OSError("boom")):
            self.assertIsNone(_free_disk_bytes(Path(".")))
...
class RunDoctorIntegrationTests(unittest.TestCase):
    ...
    @patch("app.doctor._free_disk_bytes", return_value=20 * 1024 * 1024 * 1024)
    def test_all_pass_produces_ready_report(
        self, mock_free, mock_writable, mock_managed, mock_identity,
    ...
    @patch("app.doctor.managed_runtime_python", return_value=Path(sys.executable))
    @patch("app.doctor._check_writable", return_value=True)
    @patch("app.doctor._free_disk_bytes", return_value=2 * 1024 * 1024 * 1024)
    def test_low_disk_space_blocks_render_ready(
        self, mock_free, mock_writable, mock_managed, mock_identity,
        mock_probe, mock_mod, mock_which, mock_deps, mock_load, mock_dirs, mock_write
    ):
        from app.doctor import run_doctor
        ...
        self.assertFalse(report["renderReady"])
        disk_checks = [c for c in report["checks"] if c["name"] == "Free disk space"]
        self.assertTrue(disk_checks)
        self.assertEqual(disk_checks[0]["status"], "FAIL")
